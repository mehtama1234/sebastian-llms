...
"$VENV_PYTHON" -m arch_adv_2026.long_context_variant_plan \
  --selector-artifact "$ROOT_DIR/artifacts/long_context/qwen3_variant_proxy_selector.json" \
  --compare-artifact "$ROOT_DIR/artifacts/long_context/qwen3_variant_proxy_compare.json" \
  --objective "$OBJECTIVE" \
  --execution-mode "$EXECUTION_MODE" \
  --artifact-out "$ROOT_DIR/artifacts/plans/qwen3-${OBJECTIVE}${SUFFIX}-variant-proxy-plan.json" \
  --stdout
