...
from .meta_verifier import meta_verify
...
def _finalize(
    ...
    return ScenarioRun(
        ...
        meta_verifier_runs=meta_verifier_runs,
...
def run_scenario(bench: Benchmark, scenario: Scenario, config: HarnessConfig) -> ScenarioRun:
    ...
    def evaluate(a: Attempt) -> bool:
        nonlocal verifier_runs, meta_verifier_runs
        verdict: VerifierOutcome = verify(a, threshold=config.confidence_threshold)
        verifier_runs += 1
        accepted = verdict.accepted
        if config.meta_verifier_enabled:
            accepted = meta_verify(a, verdict, threshold=config.confidence_threshold).accepted
            meta_verifier_runs += 1
        return accepted
    ...
    if not config.verifier_enabled:
        ...
        a = attempt(seeds)
        return _finalize(
            scenario, config, executed, a, a.claimed_success, escalated=False, verifier_runs=0, meta_verifier_runs=0
        )
