...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect the official Qwen3 0.6B reference model and persist a config snapshot."
    )
    parser.add_argument(
        "--model-id",
        default=DEFAULT_MODEL_ID,
        help="Hugging Face model identifier to inspect.",
    )
    parser.add_argument(
        "--artifact-dir",
    ...
    parser.add_argument(
        "--config-only",
        action="store_true",
        help="Skip full model weight loading and record only config/tokenizer-level metadata.",
    )
    return parser
...
def _load_snapshot(model_id: str, *, config_only: bool) -> ReferenceSnapshot:
    import transformers
...
def _write_snapshot(snapshot: ReferenceSnapshot, artifact_dir: Path) -> Path:
    artifact_dir.mkdir(parents=True, exist_ok=True)
    artifact_path = artifact_dir / "qwen3_0_6b_reference_snapshot.json"
    artifact_path.write_text(json.dumps(asdict(snapshot), indent=2) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    snapshot = _load_snapshot(args.model_id, config_only=args.config_only)
    artifact_path = _write_snapshot(snapshot, Path(args.artifact_dir))
